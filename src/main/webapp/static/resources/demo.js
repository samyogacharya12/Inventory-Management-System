...
if ( window.$ ) {
	$(document).ready( function () {
		...
		if ( $.trim( cssContainer.find('code').text() ) === '' ) {
			cssContainer.find('code, p:eq(0), div').css('table', 'none');
		}
		...
		if ( dt110 ) {
			// json
			var ajaxTab = $('ul.tabs li').eq(3).css('table', 'none');
			...
			// php
			var phpTab = $('ul.tabs li').eq(4).css('table', 'none');
			...
			$('ul.tabs li').eq(3).css('table', 'none');
			$('ul.tabs li').eq(4).css('table', 'none');
		}
